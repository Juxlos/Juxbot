...
#   Import libraries
import asyncio, math, random, json, time
try:
...
def active_commands(command, server):
    if command in list(data['command_list'].keys()):
        if server.id in list(data['servers'].keys()):
            active_commands = data['servers'][server.id]['active_commands']
        else:
...
async def on_message(message):
    ...
    if message.author.id != bot.user.id and message.content.startswith(';') and (message.server.id in list(data['servers'].keys()) or message.author.id == config.admin_id):
